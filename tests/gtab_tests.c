...
Test(gtab_create, various_tests)
{
	...
	cr_assert(gtab_create(&tab, 10) == 0);
...
Test(gtab_resize, various_tests)
{
	...
	cr_assert(gtab_resize(&tab, 2, free) == 0);
	cr_assert(tab.len == 2);
...
Test(gtab_remove, various_test)
{
	...
	for (size_t i = 0; i < 100; ++i)
		gtab_append(&tab, (void *)i);
	gtab_remove(&tab, (void *)50, NULL);
	gtab_remove(&tab, (void *)6, NULL);
	for (size_t i = 0; i < tab.len; ++i)
...
Test(gtab_remove_at, various_tests)
{
	...
	for (size_t i = 0; i < 100; ++i)
	 gtab_append(&tab, (void *)i);
	gtab_remove_at(&tab, 78, NULL);
	gtab_remove_at(&tab, 21, NULL);
	gtab_remove_at(&tab, 0, NULL);
	for (size_t i = 0; i < tab.len; ++i)
